...
class stwRefreshAll {
    ...
    function __construct() {
        ...
        add_action('stw_cron_cache', array($this, 'prep_delete_cache'));
    ...
    function OptionsPage() {
        ...
        if (isset($_POST["stw_timestamp_check"])) {
			$t = $_POST["stw_timestamp_check"];
			update_option('stw_timstamp_options_page', $t);
			switch($t){
					case 'every_ten':
						$time = 660;						
					break;
            ...
            update_option('stw_time_check_images', $time);
        ...
        $to = get_option('stw_timstamp_options_page');
        ?>
        <div class="wrap" style="text-align:center">
			<a href="https://shrinktheweb.com/">
			...
			if($this -> has_portfolio_plugin)
				echo '<b>WP-Portfolio Plugin detected</b>';
            ...
            <h3>Select an option to check if all the Thumbnails have been updated(Default: 10 Minutes) </h3> 
            <form action="" method="post" >
                <select name="stw_timestamp_check" id="">
                    <option value="every_ten"  <?php if ($to == 'every_ten') echo 'selected="selected"'; ?>>Every 10 Minutes</option>
                    <option value="every_thirty" <?php if ($to == 'every_thirty') echo 'selected="selected"'; ?> >Every 30 Minutes</option>
                    <option value="every_hour" <?php if ($to == 'every_hour') echo 'selected="selected"'; ?> >Every Hour</option>
                    <option value="every_three" <?php if ($to == 'every_three') echo 'selected="selected"'; ?> >Every 3 Hours</option>
                    <input class="button-primary" type="submit" value="Save" />
                </select>
            </form>
        ...
        if (isset($_POST['refresh_now'])) {
			if(!$this->has_plugin && !$this->has_theme && !$this->has_portfolio_plugin ){
				echo "<br/><b>  STW Plugin, Directorypress or WP-Portfolio plugin  is not activated </b> <br/> No thumbnails to refresh!";
				return;
			}
			
			if(!$this->has_stw_credential()){
				echo "<br/><b> STW Access key or Secret Key is not set in STW plugin or Directorypress theme or in WP-Portfolio plugin </b> <br/> ";
				return;
			}
        ...
        if(isset($_POST['clear_cache'])){
			$this->delete_cache();
			echo "<br/><b>All cached images have been deleted";
    ...
    function has_stw_credential(){
		...
		if($this ->has_theme){
			if( (ACCESS_KEY  == '') || (SECRET_KEY == '') ){
				...
				if($this->has_plugin_credential())
					...
					$this->access_id = ACCESS_KEY;
					$this->secret_id = SECRET_KEY;
				return true;
				}
		}
		...
		if($this->has_portfolio_plugin){
				...
				$id = stripslashes(get_option('WPPortfolio_setting_stw_access_key'));     
				$secret = stripslashes(get_option('WPPortfolio_setting_stw_secret_key'));
				if(strlen($id) < 2 || strlen($secret) < 2 )
					return false;
				$this->access_id = ACCESS_KEY;
				$this->secret_id = SECRET_KEY;
				return true;
		...
		function has_plugin_credential(){
			...
			if($this->has_plugin){		
				...
				if(($allSettings['stwwt_access_id'] == false ) || ($allSettings['stwwt_secret_id'] == '' ))
					...
					$this->access_id = $allSettings['stwwt_access_id'];
					$this->secret_id = $allSettings['stwwt_secret_id'];
					return true;
				}
			}
...
// end of init var
    ...
    function start_refresh() {
		update_option('stw_refresh_last_timestamp', time());
...
//searching for the thumb shortcode
    ...
    function write_in_db($links) {
       ...
       //exit;
        ...
        function getLegacyResponse($sSearch, $s) {
    ...
    //check the image last updated values
    function check_again(){
		$this->has_stw_credential();
		global $wpdb;
		$last = (int) get_option('stw_refresh_last_timestamp');
		
		$links = $wpdb->get_results("select * from {$this->table} order by id Desc limit 10");
		
		foreach($links as $link){
			$args = json_decode( $link->args, true ); 			
			unset($args['stwredo']);			
			$args['stwurl'] = $link ->url;			
			$fetchURL = urldecode("http://images.shrinktheweb.com/xino.php?" . http_build_query($args));
            ...
            if (!is_wp_error($resp)){				
				$ts_last = (int) trim ($this-> getLegacyResponse( 'ResponseTimestamp' , $resp['body'] ));
					
				if($ts_last == 0)continue;
				//echo $ts_last, '<br/>', $last;
    ...
    function prep_delete_cache(){
			if($this->check_again()){			
				$time = (int) get_option('stw_time_check_images');
				$time = ($time == 0)? 660 : $time;
				wp_schedule_single_event( time() + $time, 'stw_cron_cache');
				return;
			}
			$this->delete_cache();
		}
